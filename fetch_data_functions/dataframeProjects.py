...
async def get_specific_project(
    hubId, projectId, projectName, projectService: ProjectsService
):
    try:
        projects_df = pd.DataFrame(
            await projectService.get_specific_project(hubId, projectId)
        )
        ...
        attributes_expanded.columns = [
            "attr_" + str(col) for col in attributes_expanded.columns
        ]
        projects_df = pd.concat(
            [projects_df.drop("attributes", axis=1), attributes_expanded], axis=1
        )
        return projects_df
    except Exception as e:
