...
async def fetch_and_normalize_derivative_obj_hierarchy(urnOfSource, guid):
    object_hierarchy_df = await get_hierarchy(urnOfSource, guid, services['derivativeService'])
    return object_hierarchy_df

async def fetch_and_normalize_derivative_items_properties(urnOfSource, guid, objectid):
    item_properties_df = await get_properties(urnOfSource, guid, services['derivativeService'], objectid)
...
fetch_functions = {
    ...
    'fetch_and_normalize_specific_projects': lambda hub_Id, project_id, project_name: asyncio.run(fetch_and_normalize_specific_projects(hub_Id, project_id, project_name)),
    ...
    'fetch_and_normalize_derivative_items_properties': lambda urnOfSource, guid, objectid: asyncio.run(fetch_and_normalize_derivative_items_properties(urnOfSource, guid, objectid))
}
